...
//function App() {
export default class Cidades3 extends React.Component {
  constructor(props) {
    super(props);
    this.state = { 
      cidades: [],
  ...
  async componentDidMount() {
    ...
    //console.log(uf);
    let teste2 = JSON.parse(localStorage.getItem("myCidade23"));
    if (teste2) {
      cid_teste = teste2.city;
    }
    let teste = JSON.parse(localStorage.getItem("myCidade13"));
    if (teste) {
      uf = teste.state;
    }
    ...
    await axios.get('https://brasil.io/api/dataset/covid19/caso/data/?is_last=True&state='+ uf +'')
      ...
      if (localStorage.getItem("myCidade13") === null) {
        localStorage.setItem('myCidade13', JSON.stringify(this.state.cidades));
      }
  ...
  onChangeCidade = e => {
    ...
    localStorage.setItem('myCidade13', JSON.stringify({"state":estado}));
  ...
  onChangeCidade1 = e => {
    ...
    if (localStorage.getItem("myCidade23") === null) {
      localStorage.setItem('myCidade23', JSON.stringify({"city":e.target.value}));
    }
  ...
  render() {
      ...
      let cid1 = JSON.parse(localStorage.getItem("myCidade23"));
      let state1 = JSON.parse(localStorage.getItem("myCidade13"));
