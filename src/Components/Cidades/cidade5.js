...
export default class Cidades2 extends React.Component {
  ...
  async componentDidMount() {
    ...
    //console.log(uf);
    let teste2 = JSON.parse(localStorage.getItem("myCidade25"));
    if (teste2) {
      cid_teste = teste2.city;
    }
    let teste = JSON.parse(localStorage.getItem("myCidade15"));
    if (teste) {
      uf = teste.state;
    }
    ...
    await axios.get('https://brasil.io/api/dataset/covid19/caso/data/?is_last=True&state='+ uf +'')
      ...
      if (localStorage.getItem("myCidade15") === null) {
        localStorage.setItem('myCidade15', JSON.stringify(this.state.cidades));
      }
  ...
  onChangeCidade = e => {
    ...
    localStorage.setItem('myCidade15', JSON.stringify({"state":estado}));
  ...
  onChangeCidade1 = e => {
    ...
    if (localStorage.getItem("myCidade25") === null) {
      localStorage.setItem('myCidade25', JSON.stringify({"city":e.target.value}));
    }
  ...
  render() {
    ...
    let cid1 = JSON.parse(localStorage.getItem("myCidade25"));
    let state1 = JSON.parse(localStorage.getItem("myCidade15"));
